        ...
        for (let i = 0; i < collects.length; i++) {
          ...
          sdk["retrieving-a-single-collection"]({ collection_slug: c.slug })
            .then(async (res) => {
              ...
              let royalty =
                (Number(res.collection.dev_seller_fee_basis_points) + Number(res.collection.opensea_seller_fee_basis_points)) / 100;
              ...
              if (alertChannel) {
                if (checkAbove && currFloor && currFloor >= c.target && currFloor != c.last_check) {
                  console.log(`[Guild ${guild.id}]: Floor Alert Found ${c.slug} • Current Floor: ${currFloor}Ξ`);
                  alertChannel.send({ embeds: [alertEmbed] });
                } else if (!checkAbove && currFloor && currFloor <= c.target && currFloor != c.last_check) {
                  console.log(`[Guild ${guild.id}]: Floor Alert Found ${c.slug} • Current Floor: ${currFloor}Ξ`);
                  alertChannel.send({ embeds: [alertEmbed] });
                }
