  ...
  return new Promise((resolve, reject) => {
    ...
    sdk["retrieving-a-single-collection"]({ collection_slug: collection_slug })
      .then(async (res) => {
        ...
        let collection_contract = collection_asset.address;
        if (!collection_contract) return reject({ status: 400, reason: "Could not find collection contract. Assets not on Ethereum are currently not supported." });
            ...
            request(
              ...
              function (error, response, body) {
                if (!error && response.statusCode == 200) {
                  ...
                  request(
                    `https://api.opensea.io/api/v1/asset/${collection_contract}/${token_id}/offers`,
                    {
                      method: "GET",
                      headers: {
                        "X-API-KEY": botconfig.OS_API_KEY,
