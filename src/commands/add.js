...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("add")
    .setDescription(
      "Add a collection to monitor its floor price: add [collection slug] [price in ETH]"
    )
    ...
    .addNumberOption((option) =>
      option
        ...
        .setDescription("Target price for collection (in ETH).")
        .setRequired(true)
    )
    .addBooleanOption((option) =>
      option
        .setName("above-target")
        .setDescription(
          "If you want to monitor for a buy in (below target price) set this to False. Defaults to True."
        )
    ),
    ...
    try {
      await sdk["retrieving-a-single-collection"]({
        ...
        .then(async () => {
          ...
          const checkAbove =
            interaction.options.getBoolean("above-target") == null
              ? true
              : interaction.options.getBoolean("above-target");
          const lastCheckDefault =
            interaction.options.getBoolean("above-target") == null
          ...
          if (res.alerts_channel == "")
            return interaction.reply({
              content:
                "Please setup an alerts channel first with /setalerts in order to monitor collections.",
              ephemeral: true,
            });
          ...
          for (var i = 0; i < collections.length; i++) {
            if (collections[i].slug == collectionSlug) {
              // If collection exists in list already, update the values
          ...
          if (!alreadyAdded) {
            // Else if collection does not exist, add it to the list
          ...
          let response = `Successfully ${change} **${collectionSlug}** with target price ${targetPrice}Ξ to monitor list.`;
          if (aliasFound) {
            response +=
              " Command utilized an alias. If you would like to delete use /delalias [alias]";
          }
