...
module.exports = {
    ...
    async execute(interaction, args, client) {
        ...
        if (lenBefore == collections.length) return interaction.reply({ content: 'Could not find collection in monitor list. Check for typos or use /getlist', ephemeral: true })

        try {
            const res = await guildSchema.findOneAndUpdate({ guild_id: interaction.guild.id }, { collections: collections })
            if (res == null) return interaction.reply({ content: 'An error occurred. Please try again.', ephemeral: true });
        } catch (err) {
